...
let burgerBtns = document.querySelectorAll('.wrapper__list-btn'),
    ...
    checkListText = document.querySelector('.wrapper__navbar-checklist-text'),
    allClose = document.querySelector('.all___close');
...
allClose.addEventListener ('click', () => cartList.classList.remove('active'))
...
function basket() {
    ...
    if(cartAmount > 0 ){
        checkListText.classList.remove('active')
       }else if (cartAmount <= 0){
        checkListText.classList.add('active')
       }
    
    cartTotalPrice.innerHTML = totalSumBurgers()
    korzinka == '' ? cartListItems.innerHTML = ` <div class="wrapper__navbar-checklis-p"><p class="wrapper__navbar-checklist-text text">Пусто</p></div>` : ''
}
